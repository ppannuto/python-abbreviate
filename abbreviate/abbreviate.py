...
class Abbreviate(object):
	...
	def abbreviate(self, string, target_len=None, len_func=lambda x: len(x),
			try_harder=False, no_matter_what=False):
		...
		The len_func argument is a function that takes a single string argument
		...
		while True:
			...
			for i in range(len(tokens)):
				...
				if len_func(working) < target_len:
...
if __name__ == '__main__':
	...
	for s in [
		...
		print("{:3} {}\n{:3} {}\n{:3} {}\n".format(
			len(s), s,
			len(a.abbreviate(s)), a.abbreviate(s),
			len(a.abbreviate(s,35)), a.abbreviate(s,35),
			))
